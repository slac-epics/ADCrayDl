...
dbLoadDatabase("$(TOP)/dbd/CrayDlApp.dbd")
CrayDlApp_registerRecordDeviceDriver(pdbbase) 
...
# CrayDlConfig(const char *portName, int maxSizeX, int maxSizeY, int dataType,
#                   int maxBuffers, int maxMemory, int priority, int stackSize)
CrayDlConfig("$(PORT)", $(XSIZE), $(YSIZE), 1, 0, 0)
dbLoadRecords("$(ADCORE)/db/CrayDl.template","P=$(PREFIX),R=cam1:,PORT=$(PORT),ADDR=0,TIMEOUT=1")

# Create a second CrayDl driver
CrayDlConfig("SIM2", 300, 200, 1, 50, 50000000)
dbLoadRecords("$(ADCORE)/db/CrayDl.template","P=$(PREFIX),R=cam2:,PORT=SIM2,ADDR=0,TIMEOUT=1")
...
# Create a standard arrays plugin, set it to get data from second CrayDl driver.
